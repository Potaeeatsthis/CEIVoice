...
export default function UserTicketDetailView({ ticket, initialComments, currentUser, isOwner, isFollowing, isGuest = false, followers = [] }: Props) {
  ...
  const isClosed = ticket.status === 'SOLVED' || ticket.status === 'FAILED';
  const canComment = !isGuest && !isClosed && ticket.status !== 'MERGED' && (isOwner || isFollowing);
  ...
  const closedStatusColor: Record<string, string> = {
    SOLVED: 'text-emerald-400', FAILED: 'text-red-400'
  };
        ...
        {ticket.status === 'MERGED' ? (
          <div className="p-4 bg-zinc-900/30 border-t border-zinc-800 flex items-center gap-3">
            ...
            <p className="text-sm text-zinc-500">
              This ticket was <span className="font-semibold text-purple-400">MERGED</span> into another ticket — please check the linked ticket for updates.
            </p>
          </div>
        ) : isGuest ? (
