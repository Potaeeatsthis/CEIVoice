...
async function getData(ticketId: string) {
  ...
  // 1. Fetch the Ticket (Direct DB access)
  ...
  // 2. Fetch Comments (Placeholder - assuming you have a comments table)
  // If no table yet, just return []
  const { data: comments } = await supabaseAdmin
    .from('comments') // Ensure this table exists in your DB!
    .select('*, user:users(full_name)')
    .eq('ticket_id', ticketId)
    .order('created_at', { ascending: true });

  // 3. Fetch All Users (Only if Admin, for the dropdown)
  let allUsers: any[] = [];
  ...
  if (!data || !data.ticket) {
    return (
      <div className="p-8 text-center">
        ...
        <Link href="/tickets" className="text-blue-400 hover:underline">
