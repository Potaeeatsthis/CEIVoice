          ...
          {tickets.map((ticket) => (
            <tr key={ticket.id} className="group hover:bg-zinc-900/30 transition-colors">
              ...
              <td className="px-6 py-4 text-zinc-400">
                {ticket.deadline ? (
                    <span className="text-zinc-300 font-mono text-xs">{new Date(ticket.deadline).toLocaleDateString()}</span>
              ...
              <td className="px-6 py-4 text-right">
                {/* 👇 CONDITIONAL ROUTING: Drafts go to Editor, others to Details */}
                <Link 
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    DRAFT: "bg-zinc-800 text-zinc-400 border-zinc-700" // Distinct style for drafts
  };
