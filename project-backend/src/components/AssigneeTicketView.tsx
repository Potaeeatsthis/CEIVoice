...
export default function AssigneeTicketTable({ initialTickets }: { initialTickets: Ticket[] }) {
  ...
  // ✨ Fetch User ID securely on the client
  useEffect(() => {
  ...
  // ✨ Fetch Unread Stats to power the Red Badges
  useEffect(() => {
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-xs whitespace-nowrap font-medium border ${styles[status] || "bg-zinc-800 text-zinc-400 border-zinc-700"}`}>{status.replace('_', ' ')}</span>;
}
